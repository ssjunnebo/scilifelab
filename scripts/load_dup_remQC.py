...
def main(stat_file, single, min, proj):
    ...
    for key, val in dict.items():
        if single:
            M_reads_aft_dup_rem = round(float(val['aft_dup_rem']['mapq >= mapq_cut (unique)'])/1000000.0,2)
        else:
            R1 = val['aft_dup_rem']['Read-1']
            R2 = val['aft_dup_rem']['Read-2']
            M_reads_aft_dup_rem = round((float(R2)+float(R1))/2000000,2)
        if M_reads_aft_dup_rem > (float(min)/2.0):
            qc_passed = True
        else:
            qc_passed = False
        print "{0}  : {1} : {2} : {3}".format(key, qc_passed ,str(M_reads_aft_dup_rem), min)
        qc_dict[key] = {"automated_qc": {
                                "qc_passed": qc_passed,
                                "qc_reason": "M reads after duplicates removed: {0}. Minimal amount: {1}".format(str(M_reads_aft_dup_rem), min)}}
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', default = False, dest = 'min',
                            help=('Min reads (M)'))
